...
{   
	u8 res=0;
	res=IIC1_Read_One_Byte_(AK8963_ADDR,AK8963_WIA);    			//读取AK8963 ID 
    vTaskDelay(20);
    
	if(res==AK8963_ID)
	{        
        printf("AK8963 I2C Connection [OK].\r\n");
		IIC1_Write_One_Byte_(AK8963_ADDR,AK8963_CNTL2,0X01);		//复位AK8963
        vTaskDelay(50);
		IIC1_Write_One_Byte_(AK8963_ADDR,AK8963_CNTL1,0X11);		//设置AK8963为单次测量
        return 0;
	}
	else 
    {
...
{
    u8 buf[6],res;  
	res=IIC1_Read_NByte_(AK8963_ADDR,AK8963_XOUT_L,6,buf);
	if(res==0)
	{
