#include "bsp_sensorfusion.h"

#define ACCZ_SAMPLE		350
#define DEG2RAD		0.017453293f	/* 度转弧度 π/180 */
#define RAD2DEG		57.29578f		/* 弧度转度 180/π */

float halfT = 0.5f;  // 采样周期的一半
float Kp = 0.4f;			/*比例增益*/
...
static void calBaseAcc(float* acc)	/*计算静态加速度*/
{
	...
	if(++cnt >= ACCZ_SAMPLE) /*缓冲区满*/
	{
		...
		if(maxError < 0.015f)
		{
			...
			GPIO_SetBits(GPIOC, GPIO_Pin_13);	/*校准通过指示灯*/
		}
...
// integrate quaternion rate and normalise四元数微分方程
